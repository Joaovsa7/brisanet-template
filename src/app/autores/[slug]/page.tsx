...
import { EmailIcon, FacebookIcon, InstapaperIcon, LinkedinIcon } from 'react-share'
...
export default async function AuthorPage({ params }: IPageProps) {
    ...
    const authorArticleDocuments = await client.getAllByType<IArticleDocumentResponse>('article', {
      fetchLinks,
        ...
        <Container size="lg">
          <section>
            <div className="flex items-center gap-6">
              <PrismicNextImage field={document.data.avatar} height={360} width={360} className="w-24 h-24 rounded-full" />

              <div>
                <h1 className="text-4xl font-bold tracking-tight">
                  {document.data.name}
                </h1>
                <p className="font-medium text-lg mt-1 block">{document.data.position}</p>
              </div>
            </div>
            ...
            {socialNetworksIsFilled && (
              <>
                <h3 className="text-xl font-bold mt-8">Fale com {document.data.name}</h3>

                <ul className="mt-4">
                  {document.data.social_networks.map((socialNetwork) => (
                    <li key={socialNetwork.name}>
                      <PrismicNextLink field={socialNetwork.link} className="w-10 h-10 bg-neutral-200 rounded-full overflow-hidden flex items-center justify-center transition-transform duration-300 hover:-translate-y-0.5">
          ...
          {articlesIsPopulated && (
            <section className="my-16">
              <h2 className="mb-8 text-3xl font-bold tracking-tight">Artigos mais recentes</h2>
